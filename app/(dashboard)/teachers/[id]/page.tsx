...
type TeacherData = {
  ...
  profile_pic?: string; // URL or base64
};

const TeacherDetailPage = ({ onClose }: { onClose: () => void }) => {
  ...
  // Fetch existing teacher if editing
  useEffect(() => {
    if (isEditMode) {
              ...
              <div className="flex flex-wrap gap-2">
                {teacherData.classes_handled.map((c, i) => (
                  <span key={i} className="bg-green-100 text-green-800 text-xs px-3 py-1 rounded-full flex items-center gap-1">
                    {c} <button type="button" onClick={() => removeClass(i)}>×</button>
                  </span>
                ))}
              </div>
            </div>
